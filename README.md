...
- **宽度 ≥ 2400px 且高度 ≥ 1600px**（4K 屏 100% 缩放）：`--maxw` 放宽到 `min(2120px, 88vw)`，标题、正文、图片高度上限都再放大一档，避免内容缩在屏幕正中一小块。
- **宽度 ≥ 1921px**（含 4K 开 150% 缩放后的 ~2555px 视口）：`--maxw` 放宽到 `min(1800px, 88vw)`，字号与图片高度同步放大。
