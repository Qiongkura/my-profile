...
- **音量条**在单曲行的右侧：默认 **80%**，拖动即时生效，点小喇叭静音 / 恢复，键盘 `←` `→` 每次 5%、`Home` / `End` 到 0 / 100%。改动写进 `localStorage` 的 `qiongkura-volume`，下次打开沿用（隐私模式下写不进去也不影响播放）
...
**歌词文本没有替你写**：商业歌曲的歌词有版权，而且无法保证逐字准确。自己从网易云 / QQ 音乐的歌词页复制带时间轴的全文，存成 `assets/audio/cha-tang.lrc`（UTF-8）即可。换成别的歌时，`data-src` 与 `data-lrc` 一起改。以后如果网易云解析插件配好了代理（`API_BASE`），也可以让歌词直接来自那边，不用本地文件。
